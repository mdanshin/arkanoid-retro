...
export class NeonRenderer{
 ...
 capsule(d){const c=this.ctx,p=POWERUPS[d.type];this.glow(p.color,d.x,d.y,76,55,.65);c.save();c.translate(d.x,d.y);const r=14;c.rotate(Math.sin(d.age*2)*.12);hex(c,0,0,r,Math.PI/6);c.fillStyle='#0a1b32';c.fill();c.strokeStyle=p.color;c.lineWidth=1.3;c.stroke();c.strokeStyle=p.color+'55';hex(c,0,0,r+4,Math.PI/6);c.stroke();c.fillStyle=p.color;c.font='600 14px Arial';c.textAlign='center';c.textBaseline='middle';c.fillText(d.type,0,1);c.restore();}
